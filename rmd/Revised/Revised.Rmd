...
The Manhattan block with the highest number of trees is Riverside Dr between 116th St and 119th St, with `r head(blocks[order(blocks$trees, decreasing = TRUE),]$trees, 1)` trees. While interesting in its own right, this doesn't take into account the distance of a block. For instance, this `block` along Riverside Dr is technically a correct `block`; it runs from 116th to 119th uninterrupted by any intersections. But it's clearly the distance of 3 grid blocks. Also take into account that avenues are spaced farther than streets. To reconcile this issue, a slightly different question should be asked: Which block has the highest density of trees? The trees-per-meter (TPM) approach, as discussed in the [first article](http://www.joewillage.com/blog/2016/3/31/analyzing-new-yorks-trees), is applied. This includes finding the distance of a block using the Haversine great-circle distance formula, then dividing the number of trees by that distance.  
