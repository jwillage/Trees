...
for (tree in 1:nrow(trees)) { 
  ...
  error = function(e){
    print(paste("error on tree", tree, "sleeping at", timestamp(quiet = T)))
    Sys.sleep(3600)
    tree <- tree - 1
...
####Clear cache for following after all trees complete. 
...
There are a lot of errors, but not too bad relative to the total number of trees mapped (1.04%). The previous post introduced k-means clustering to group a block of trees together. The clustering method helps reduce the amount of manual labor, but the number of clusters has to be conservative, and clusters have to be verified. The starting number of clusters is obtained by dividing the number of errors by the median number of trees per block, resulting in 168 clusters. The idea is that each cluster will map to a block. 
...
![](Figs/manh errors-1.png)

As expected, there aren't many errors in midtown, where the grid system is working. The majority of errors pop up near the highways and bridges. It will take zooming in on each neighborhood and labeling the clusters to see if clusters can be combined or need to be split further.  
