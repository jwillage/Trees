...
The approach involves the Geonames Find Nearest Intersection (FNI) API. Each segment previously returned from FNB will have it's two endpoints passed into FNI. The order of rules is as follows:  
...
1) Segments with distinct intersections are possible complete blocks. Skip next step.
1) Segments which return the same intersection for both endpoints are marked as incomplete. The endpoint with the least `distance` to the intersection is considered for a potential block-end.  
2) Endpoints that are within X `distance` of their nearest intersection are marked as a block end. X is close to 0.     
2) Mark block as complete if it has two block ends. 
3) If block is incomplete, repeat for remaining segments.   
...
fni.parms1 <- paste0("lat=", lines[lines$i == 1, "lat"][1], 
                     "&lng=", lines[lines$i == 1, "lon"][1])
int1 <- fromJSON(file = paste0(fni.url, fni.parms1))
fni.parms2 <- paste0("lat=", tail(lines[lines$i == 1, "lat"], 1), 
                     "&lng=", tail(lines[lines$i == 1, "lon"], 1))
int2 <- fromJSON(file = paste0(fni.url, fni.parms2))
if (int1$intersection$street1 == int2$intersection$street1 & 
    int1$intersection$street2 == int2$intersection$street2) {
  if (int1$intersection$distance == min(int1$intersection$distance, int2$intersection$distance)) {
    if (int1$intersection$distance < 0.001) {
      rbind(blocks, 
            c(length(blocks$id) + 1, int1$intersection$street1, int1$intersection$street2,
              int1$intersection$lat, int1$intersection$lng))
    } else{ }
  } else{
      if (int2$intersection$distance < 0.001) {
      rbind(blocks, 
            c(length(blocks$id) + 1, int2$intersection$street1, int2$intersection$street2,
              int2$intersection$lat, int2$intersection$lng))
    } else { }
  }
    
} else{
  # TODO repeat for case int1 != int2
}
```
...
The first endpoint of the segment is indeed on the intersection. But the other endpoint has distance > 0. 
...
blocks <- rbind(blocks, 
                c(length(blocks$id) + 1, int1$intersection$street1, int1$intersection$street2,
                  int1$intersection$lat, int1$intersection$lng))
names(blocks) <- c("id", "street1", "street2", "lat", "lon")
```

Check if the block can be marked as complete.   
#TODO how to identify blocks. Consider a temp object where all block ends (1 or 2) are part of the POI



```r
if (nrow(blocks[blocks$id == 1, ]) == 2) {
  blocks[blocks$id == 1, "complete"] <- TRUE
}
```
