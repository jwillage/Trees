...
And the catch-all group includes only `r length(err)` points. The first one looks like this. 

```{r}
treeMap[err, ]
drawLines(treeMap[err[1], "lat"], treeMap[err[1], "lon"])
```

Here, even the map layer fails to capture the actual streets. The first 2 error points lie on Lillian Wald Dr, a block that actually loops around PS 188. Those streets are made up of segments 1, 7, and a segment between 7 and the middle of 9. An accurate row for these points would have primary street as Lillian Wald Dr, and both cross streets as E Houston St. 

Save blocks/treeMaps.1967 and read from them and plug the # errors in from that.