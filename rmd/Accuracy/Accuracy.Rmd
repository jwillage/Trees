...
```
And note that all of these points mapped to the block of Grand b/w Clinton and Suffolk, when most actually lie on Clinton St. The southmost point mapped to this block is tree 165907 (`r head(block.35[order(block.35$lat), "lat"], 1)`, `r head(block.35[order(block.35$lat), "lon"], 1)`). Tree 165907 is reverse geocoded as 410 Grand, with the ArcGIS API. One alternative is retrieving the full tree data from the Tree Map website. That returns the nearest address as 187 Clinton. Much more accurate, but not extensible. 
...
```{r include = FALSE}
treeMap <- readRDS("rds/treeMap.985.rds")
...
tmpBlock <- NULL
treeMap$blockId <- NULL
for (tree in 662:nrow(treeMap)) { #nrow(treeMap)
  segments <- getSegments(treeMap[tree, "lat"], treeMap[tree, "lon"])
  primary <- segments[1, "street"]
  tmpBlock <- data.frame()
  for (segment in 1 : nrow(segments)) {
...
```{r include = FALSE}
treeMap <- readRDS("rds/treeMap.1976.rds")
...
The remaining problem-points fell into the catch-all group (`err`), which includes only `r length(err)` points. The first one looks like this. 
...
Rather than manually correctly all of these, clustering will be group points close together, and the clusters can be manually corrected. That will knock the work down from `r length(err)` to 7 clusters.  
